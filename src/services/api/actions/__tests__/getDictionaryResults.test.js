...
describe('getDictionaryResults action', () => {
	...
	it(`should match getDictionaryResults action for keyword "cancer"`, () => {
		const keyword = 'cancer';
		const expectedAction = {
			interceptorName: 'glossary-api',
			method: 'GET',
			endpoint: `{{API_HOST}}/Terms/search/{{DICTIONARY_NAME}}/{{AUDIENCE}}/{{LANGUAGE}}/${encodeURI(keyword)}${queryString}`,
		};
